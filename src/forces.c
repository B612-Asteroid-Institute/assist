    ...
    for (int k=0; k < ASSIST_BODY_NPLANETS + spl_num; k++){
        ...
        for (int j=0; j<N_real; j++){
            ...
            if (i==ASSIST_BODY_EARTH && _r < impact_distance && assist->recorded_impacts->impact_jd[j]==0){
                ...
                // Record the impact
                assist->recorded_impacts->impact_jd[assist->recorded_impacts->N_impacts-1] = t + jd_ref;
                assist->recorded_impacts->impact_dist[assist->recorded_impacts->N_impacts-1] = _r;
                assist->recorded_impacts->hash[assist->recorded_impacts->N_impacts-1] = particles[j].hash;
                assist->recorded_impacts->x[assist->recorded_impacts->N_impacts-1] = particles[j].x;
                assist->recorded_impacts->y[assist->recorded_impacts->N_impacts-1] = particles[j].y;
                assist->recorded_impacts->z[assist->recorded_impacts->N_impacts-1] = particles[j].z;
                assist->recorded_impacts->vx[assist->recorded_impacts->N_impacts-1] = particles[j].vx;
                assist->recorded_impacts->vy[assist->recorded_impacts->N_impacts-1] = particles[j].vy;
                assist->recorded_impacts->vz[assist->recorded_impacts->N_impacts-1] = particles[j].vz;
            }
